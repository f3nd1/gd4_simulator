...
- **Google rejects the exchange with a redirect_uri mismatch / invalid_grant 400** — this was hit on first real use. The fix is already in the code: the popup flow (GIS `initCodeClient`, `ux_mode: "popup"`) delivers the code with no redirect, so the server-side exchange must use the literal `"postmessage"` as its `redirect_uri`, not the page origin (the origin isn't a registered "Authorized redirect URI"). The Edge Function hardcodes `"postmessage"`; nothing to configure. If you somehow still see this after redeploying the latest function, confirm the deployed function contains `redirect_uri: "postmessage"`.
