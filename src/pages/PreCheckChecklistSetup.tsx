...
export function PreCheckChecklistSetup() {
  ...
  const selectedReq = GD4_REQUIREMENTS.find((r) => r.id === selectedItemId);
  const selectedSubCrit = GD4_SUB_CRITERIA.find((s) => s.id === selectedReq?.subCriterionId);
  const rows = checklists[selectedItemId] ?? [];

  // Group items by sub-criterion so the picker mirrors the audit's real
  // structure (all 29 sub-criteria, 2.2/4.2 expanding to 2 items each).
  const grouped = useMemo(() => {
    return GD4_SUB_CRITERIA.map((sc) => ({
      subCrit: sc,
      items: GD4_REQUIREMENTS.filter((r) => r.subCriterionId === sc.id),
    }));
  }, []);

  function startEdit(d: ChecklistItemDef) {
  ...
  function submit() {
    ...
    if (editingDefId) {
      updateItem(selectedItemId, editingDefId, payload);
      setEditingDefId(null);
    } else {
      addItem(selectedItemId, payload);
    }
    setForm(EMPTY_FORM);
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      <Card>
        ...
        <label style={{ display: "block", marginBottom: 10 }}>
          ...
          <select
            ...
            onChange={(e) => { setSelectedItemId(e.target.value); cancelEdit(); }}
            style={{ ...inputStyle, marginTop: 3, width: "100%" }}
          >
            {grouped.map(({ subCrit, items }) => (
              <optgroup key={subCrit.id} label={`${subCrit.id} — ${subCrit.title}`}>
                {items.map((r) => <option key={r.id} value={r.id}>{r.id} — {r.requirement}</option>)}
              </optgroup>
            ))}
          </select>
        </label>
        ...
        {selectedSubCrit && (
          <p style={{ fontSize: 11.5, color: "#94a3b8", marginTop: -4 }}>{selectedSubCrit.description}</p>
        )}
      </Card>

      <Card>
        <h3 style={{ marginTop: 0, fontSize: 14 }}>
          Checklist items for {selectedItemId} ({rows.length})
        </h3>
        {rows.length === 0 && (
          <p style={{ fontSize: 12.5, color: "#94a3b8" }}>No checklist items defined for this item yet — add one below.</p>
        )}
        {rows.length > 0 && (
          <table style={{ marginBottom: 4 }}>
            <thead>
              <tr><th>Title</th><th>Mode</th><th>Detection</th><th>Source</th><th>Status</th><th></th></tr>
            </thead>
            <tbody>
              {rows.map((d, i) => (
      ...
      <Card>
        <h3 style={{ marginTop: 0, fontSize: 14 }}>{editingDefId ? `Edit item — ${selectedItemId}` : `Add item — ${selectedItemId}`}</h3>
