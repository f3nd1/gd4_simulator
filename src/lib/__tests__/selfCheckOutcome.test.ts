...
import { outcomePassGate, outcomeDimensionState, outcomePassTally, NOT_LINKED_REASON } from "../selfCheckOutcome";
...
// case where letting the pass run would publish a Band 1 about an area nobody
// looked at.
describe("the results-and-review folder gate", () => {
  it("runs the pass when at least one file was actually read", () => {
    expect(outcomePassGate("folder123", { listed: 4, read: 4 })).toEqual({ run: true });
    expect(outcomePassGate("folder123", { listed: 9, read: 1, failed: ["a.pdf", "b.pdf"] })).toEqual({ run: true });
  });

  it("refuses when no folder is linked", () => {
    for (const link of [undefined, null, ""]) {
      const g = outcomePassGate(link, { listed: 0, read: 0 });
      expect(g.run).toBe(false);
      expect(g.run === false && g.reason).toBe(NOT_LINKED_REASON);
    }
  });

  it("refuses on a linked but empty folder", () => {
    const g = outcomePassGate("folder123", { listed: 0, read: 0 });
    expect(g.run).toBe(false);
    expect(g.run === false && g.reason).toMatch(/no files in it/i);
  });

  // THE case: a folder that lists files and fails to read every one of them
  // looks like a working folder from the outside. It must be gated exactly as
  // hard as an empty one.
  it("refuses when the folder lists files but every single one fails to read", () => {
    const g = outcomePassGate("folder123", { listed: 6, read: 0, failed: ["KPI 2025.pdf", "minutes.docx", "CAP log.xlsx", "d.pdf"] });
    expect(g.run).toBe(false);
  ...
  it("reads naturally for a single unreadable file", () => {
    const g = outcomePassGate("folder123", { listed: 1, read: 0, failed: ["scan.pdf"] });
    const reason = g.run === false ? g.reason : "";
  ...
  // A read count can only come from a real extraction, but a negative or
  // nonsense count must never be read as "something was read".
  it("treats a non-positive read count as nothing read", () => {
    expect(outcomePassGate("folder123", { listed: 3, read: 0 }).run).toBe(false);
    expect(outcomePassGate("folder123", { listed: 3, read: -1 }).run).toBe(false);
  });
});

describe("what the page says about the two dimensions", () => {
  it("says not linked when there is no stored pass at all", () => {
    expect(outcomeDimensionState(undefined)).toEqual({ state: "not-linked", reason: NOT_LINKED_REASON });
  });

  it("names the stored reason when the folder could not be read", () => {
  ...
  it("a skipped reason wins over rows that somehow survived", () => {
    expect(outcomeDimensionState({ rows: [{}], skippedReason: "folder empty" }).state).toBe("not-read");
  });
});
