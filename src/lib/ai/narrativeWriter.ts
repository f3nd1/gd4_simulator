...
export async function runNarrativeWriter(input: NarrativeInput, settings: AISettings): Promise<NarrativeWriterResult> {
  ...
  const sys =
    ...
    // 2026-07-19 QA batch — items 4/5/6/9 fold into this one clause:
    //   4: the action must close the SPECIFIC gap named in its own weakness;
    //   5: no invented requirements beyond the supplied descriptor;
    //   6: direct imperative verbs when the action is necessary (no hedging);
    //   9: no exhaustive-coverage demand unless the requirement is inherently
    //      complete.
    "\"requiredAction\" — a short, direct statement of what further evidence or records would move THIS dimension up one band, derived from and directly closing the SPECIFIC gap named in this dimension's own weakness. Name the control required, who is responsible, when it must occur and the evidence to retain. When the action is genuinely necessary to reach the next band, open with a direct imperative verb (Maintain, Document, Compile, Record, Require, Add) — do NOT hedge with \"management could\", \"it would be advisable\" or \"may compile\"; reserve softer wording only for a genuinely optional suggestion. The action MUST match its weakness: if the gap is that approval did not occur before implementation, require a named approver, an approval date and confirmation that approval preceded implementation — not merely monitoring or effectiveness checks after the fact. Do NOT invent a requirement the rubric does not state: demand a control (for example version history, ISO or GD4 cross-linkage, or efficiency-focused redesign) ONLY when the supplied next-band descriptor names it AND the evidence gap supports it. Do NOT require exhaustive coverage (\"for every partner\", \"for each item\") unless the requirement is inherently complete (for example every partner agreement must exist); where the check is verification-based (interviews, spot checks) a risk-based or sampled approach is sufficient. Do NOT reuse a stock opening such as \"To reach Band 3\" or \"To reach Band 4\" across dimensions — tailor each to its own requirement. Provide it for EVERY dimension below Band 5, whether its rows are strengths or weaknesses; omit it only when the dimension is already at Band 5. " +
