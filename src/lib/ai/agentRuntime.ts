...
SPECIFIC-PROMISE RULE (general — applies to every promise check, not tied to any one line). "evidenced" requires a record showing THAT SPECIFIC commitment carried out — its named mechanism, frequency, scope, role, timeline or threshold. A record of a RELATED or ADJACENT activity that does not cover the specific promise is "not evidenced", not a partial pass — e.g. a promise to MONITOR the implementation timelines of corrective actions is NOT evidenced by records that corrective-action plans exist and are tracked, unless a record actually shows that monitoring-timeline check running. A promise the PPD documents but that no given record shows carried out is "not evidenced" (the "documented but not implemented per PPD" gap), never softened to "evidenced" or credited toward a "Partial" merely because the topic area has some activity. Reserve "Partial" for genuine partial evidence OF THE SPECIFIC promise (done for part of the required scope, not all) — not for adjacent activity that leaves the named promise unproven.
