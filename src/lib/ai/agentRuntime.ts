...
{"lines": [{"text": string, "clause": string, "sourceRef": string, "sourceType": "describeShow"|"note"|"expectedEvidence", "sourceText": string, "apsrDimension": "Approach"|"Processes"|"Systems & Outcomes"|"Review"}], "rejectedIdeas": [{"text": string, "reason": string}]}${buildSystemPrompt("checklistScoring")}`;
...
Respond with JSON only: {"observation": string, "criteria": string, "effect": string}.${buildSystemPrompt("findingWriter")}`;
...
Cite the exact chunk ID(s) from document headers (e.g. "C001") in chunkIds. Leave chunkIds empty if no chunk directly supports the coverage verdict.${buildSystemPrompt("evidenceReview")}${domainBlock}
...
Cite the exact chunk ID(s) from document headers (e.g. "C001") in chunkIds. Leave chunkIds empty if no chunk directly supports the verdict.${buildSystemPrompt("evidenceReview")}${domainBlock}
...
Cite chunk IDs from document headers in chunkIds. Leave chunkIds empty if no chunk directly supports a true verdict.${buildSystemPrompt("evidenceReview")}${domainBlock}
