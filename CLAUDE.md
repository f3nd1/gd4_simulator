...
|---|---|---|---|---|
| `useWorkspaceStore` | Main store: cycle, auditors, folders, audit runs (`auditRunHistory`), Option A results, findings (`customFindings`), closures, calibration memories, human-decision log, `fileTextCache`, snapshots | `ucc-gd4-workspace:v3` | **13** | workspaceStorage |
...
| `useChecklistVerdictStore` | Per-check verdicts from the Audit Checklist Library pass, keyed `checkId::subCriterionId::bucket`; written by BOTH audit paths, display-only. 800 entries x (2000-char rationale + 1000-char quote) could fill a browser's whole 5 MB quota, so this is the ONE key in `NO_LOCAL_MIRROR` (`supabaseStorage.ts`): full text to Supabase, no localStorage copy, while Supabase is reachable. Caps stay where they are; the text is recoverable nowhere else | `ucc-gd4-checklist-verdicts:v1` | 0 | workspaceStorage (no local mirror) |
| `useFileTextCacheStore` | Extracted document text, keyed `driveFileId:modifiedTime`, so a reload does not re-download and re-OCR every file. The SECOND key in `NO_LOCAL_MIRROR` for the same reason as the verdicts store: one folder of scanned PDFs would fill a browser's 5 MB quota. An entry is the file's COMPLETE text or absent, never truncated (a shortened entry would make a re-run assess less than the first run, silently); oversized entries (>600k chars) are skipped, and the 4M-char ceiling evicts whole entries oldest-first | `ucc-gd4-file-text-cache:v1` | 0 | workspaceStorage (no local mirror) |
...
- `agentRuntime.ts` — `runLiveFolderAudit()`, `FOLDER_DOC_CAP = 60_000` (staged/full paths only — Option A has no doc cap; it slides 55k-char windows with 5k overlap, 8 lines per batch). Exports the staged passes (`runStagedPolicyAudit` / `runStagedEvidenceAudit` / `runStagedOutcomeReviewAudit` + `buildStagedApsr`) and Option A (`runPPDRequirementsReview` / `runEvidenceAssessment`). **Option A is two-pass (extract → judge)**: Pass 1 reads each window and returns candidate passages only (no verdicts); every candidate is verified deterministically (`quoteExistsInSource`/`verifyClauseRef`) and pooled across windows; Pass 2 judges each line ONCE from the verified passages alone (it never sees the document) — there is no cross-window verdict merge. A line whose extraction cleanly finds nothing (0 raw candidates) is decided deterministically in code (PPD: "Not documented"; evidence: by the PPD-verdict + promise-count procedure), never AI-guessed on empty input — but a line whose candidates were returned and ALL failed verification is "Not assessed" (extraction defect), never a fabricated gap; every row carries `extractionStats` (raw vs verified counts) and the Lab drill-in notes show them. `PPD_BOUNDARY_RULES`/`EVIDENCE_BOUNDARY_RULES` pin deterministic Met/Partial rules for the line patterns the consistency baseline showed flip-flopping; the judge prompts repeat the core rubric at the very END (recency). The `ppdReview` skill module uses a policy-documentation BASE override in `skills.ts` (no external-auditor/evidence-standards — their "records absent = unverified" rules contradict a documentation-only pass). Citation-gap downgrade: any positive verdict with no cited chunk is code-level downgraded, never trusted.
